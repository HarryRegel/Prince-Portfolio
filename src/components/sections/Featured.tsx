  ...
  useEffect(() => {
    ...
    getProjects()
      .then((data) => {
        setProjects(
          (data || [])
            .filter((project) => project.platform === "ai")
            .map((project) => ({
              ...project,
              platform: ["blockchain", "ai", "tools", "researchgate"].includes(project.platform)
                ? (project.platform as "blockchain" | "researchgate" | "ai" | "tools")
                : "ai", // Default to "github" if the platform is invalid
              source: project.source || "", // Provide a default empty string for missing source
            }))
        );
      })
      .catch(() => {
